...
const areasEspecificas = {
    DIRECCION_GENRAL_DE_GOBIERNO_Y_ASUNTOS_JURIDICOS: [
    ...
    DIRECCION_GENRAL_DE_ADMINISTRACION: [
    ...
    DIRECCION_GENRAL_DE_SERVICIOS_URBANOS: [
    ...
    DIRECCION_GENRAL_DE_OBRAS_Y_DESARROLLO_URBANO: [
    ...
    DIRECCION_GENRAL_DE_DESARROLLO_SOCIAL_Y_BIENESTAR: [
    ...
    DIRECCION_GENRAL_DE_DESARROLLO_ECONOMICO_Y_RURAL: [
    ...
    DIRECCION_GENRAL_DE_PARTICIPACION_CIUDADANA: [
...
nombreSolicitante.addEventListener('blur', function() {
    ...
    // Expresión regular para MAYÚSCULAS sin acentos (permite Ñ y espacios)
    const regex = /^[A-ZÑ\s]+$/;
    ...
    // Verificar si hay acentos (mayúsculas o minúsculas)
    const tieneAcentos = /[ÁÉÍÓÚáéíóú]/.test(nombre);
    
    // 1. Primero revisa si hay acentos (MUESTRA ALERTA)
    if (tieneAcentos) {
        alert('Nombre incorrecto: No se permiten acentos.');
        mensajeValidacion.textContent = '❌ No use acentos';
        mensajeValidacion.style.color = 'red';
        this.focus();
        return; // Termina la función aquí
    }
    
    // 2. Si no hay acentos, valida mayúsculas y nombre completo (SOLO MENSAJE, NO ALERT)
    if (!regex.test(nombre) || palabras.length < 2) {
        mensajeValidacion.textContent = '❌ Ingrese nombre completo en MAYÚSCULAS';
        mensajeValidacion.style.color = 'red';
        this.focus();
    } else {
